...
запрет бронирования при нулевой цене

корректное отображение занятых мест по дате

Отдельно тестируется модель MovieSession как единый источник расчёта стоимости.





<h2>Дополнения к административной панели</h2>

<p>
    В ходе разработки административная часть была значительно улучшена.  
    Работа с залами, ценами и сеансами стала удобнее, а поведение интерфейса — быстрее и предсказуемее.  
    Ниже перечислены ключевые изменения.
</p>

<h3>Работа со схемой зала</h3>

<p>
    Теперь изменение кресел происходит локально: при клике тип кресла меняется только в интерфейсе,  
    а на сервер отправляется один запрос — когда нажимается кнопка «Сохранить».  
    Это позволяет избежать множества мелких запросов и делает работу схемы зала заметно быстрее.
</p>

<p>
    После сохранения сервер возвращает обновлённые данные, и схема перерисовывается без перезагрузки страницы.
</p>

<h3>Настройка цен</h3>

<p>
    Цены теперь сохраняются полностью через AJAX. Страница не перезагружается,  
    используется обычное сообщение о том, что данные успешно сохранены.
</p>

<h3>Удаление сеанса</h3>

<p>
    При клике по сеансу теперь открывается всплывающее окно подтверждения удаления.  
    В нём отображается название фильма, а при подтверждении выполняется удаление через DELETE-запрос.  
    После успешного удаления сеанс исчезает из сетки.
</p>

<h3>Проверка пересечения сеансов</h3>

<p>
    Перед созданием или изменением сеанса выполняется строгая проверка:  
    если сеанс пересекается по времени с уже существующим, выводится обычный alert.  
    Так администратор сразу понимает, что время занято.
</p>

<h3>Открытие продаж</h3>

<p>
    Кнопка «Открыть продажу билетов» теперь полностью рабочая.  
    Выполняется AJAX-запрос, сервер проверяет наличие открытых залов  
    и устанавливает флаг <code>sales_open = true</code> через модель Setting.
</p>

<p>
    Если залы закрыты — пользователь получает аккуратное сообщение об ошибке.  
    Если всё в порядке — выводится подтверждение, и продажа считается открытой.
</p>
